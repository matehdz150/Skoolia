...
export default function ParentsSidebar({ active = 'info' }: Props) {
  const Item = ({ icon, label, href, isActive, badge }: { icon: JSX.Element; label: string; href: string; isActive?: boolean; badge?: number }) => (
    ...
    >
      ...
      {typeof badge === 'number' && (
        <span className={`min-w-6 rounded-full px-2 text-center text-xs ${isActive ? 'bg-white/20' : 'bg-slate-100 text-slate-600'}`}>{badge}</span>
      )}
    </div>
  );

  return (
    <aside className="flex w-full max-w-80 flex-col gap-4">
      ...
      <div className="rounded-3xl bg-indigo-600 p-6 text-white shadow-indigo-300">
        <h3 className="text-sm font-bold opacity-90">Sugerencias IA</h3>
        <p className="mt-2 text-sm opacity-90">Personalizamos Skoolia según los intereses de Carlos.</p>
        <button className="mt-4 w-full rounded-2xl bg-white/10 px-4 py-2 text-sm font-bold hover:bg-white/20">Ver recomendaciones</button>
      </div>
    </aside>
  );
}
